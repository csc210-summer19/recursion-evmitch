 ...
 *
 * @author Rick Mercer and Evan Cohen
 */
...
public class RecursionFun {
	...
	public String intWithCommas(int n) {
		...
		//formats it correctly, not srue if i can use this for recurison but it works
	...
	private void reverseArray(int[] x, int start, int len) {
		if (start < len) {
			int tmp = x[start];
			x[start] = x[len];
			x[len] = tmp;
			reverseArray(x, ++start, --len);
		}
	}
	...
	public boolean isSorted(int[] var) {
		int len = var.length;
		if (len == 0 || len == 1) {
			return true;
		}
		if (var[len - 2] > var[len - 1]) {
			return false;

		}
		return isSorted(Arrays.copyOfRange(var, 0, len - 2));
	}
	...
	public boolean found(String search, String[] strs) {
	  ...
	  if (strs[n-1].contentEquals(search)) {
		  return true;
	  }
